...
pub fn EncryptionAlgorithm(matrix: &mut [u8; 16], key: &super::Key) {
  match key {
      super::Key::Key128bit(key) => {
...
pub fn DecryptionAlgorithm(matrix: &mut [u8; 16], key: &super::Key) {
  match key {
      super::Key::Key128bit(key) => {
          AddKey(matrix, &key.key[160..176]);
