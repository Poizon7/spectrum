        ...
        if let Some(init_key) = &self.init_key {
            ...
            match key {
                ...
                Key::Key256bit(mut key) => {
                    for byte in match init_key {
                        InitKey::Bit256(init_key) => init_key,
                        _ => panic!("Missmatch between init key and key"),
                    } {
                        key[i as usize] = *byte;
                    }
    ...
    fn encrypt(&self, message: &mut [u8]) -> Result<Vec<u8>, CryptoError> {
        match &self.key {
            Some(key) => {
                ...
                for mut matrix in message {
                    let mut iter = matrix.to_vec();
                    if matrix.len() < 16 {
                        while iter.len() < 16 {
                            iter.push(0);
                        }
                        matrix = iter.as_mut_slice();
                    }
                    AES::encryption_algorithm(matrix, key);
...
// Shared functions
impl AES {
    fn add_key(matrix: &mut [u8], key: &[u8]) {
        for i in 0..16 {
            matrix[i] ^= key[i];
        }
    }
...
impl AES {
    ...
    fn encryption_algorithm(matrix: &mut [u8], key: &Key) {
        match key {
            Key::Key128bit(key) => {
...
impl AES {
    ...
    fn reverse_shift_rows(matrix: &mut [u8]) {
        let temp = matrix[7];
    ...
    fn reverse_mix_columns(matrix: &mut [u8]) {
        for i in 0..4 {
            let c = [
                matrix[i * 4],
                matrix[(i * 4) + 1],
    ...
    fn decryption_algorithm(matrix: &mut [u8], key: &Key) {
        match key {
            Key::Key128bit(key) => {
                AES::add_key(matrix, &key[160..176]);
