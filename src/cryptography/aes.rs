extern crate rand;

use crate::cryptography::CryptographicAlgorithm;
use crate::format::vec_to_array16;
use crate::math::finite_field_multiplication;

mod aes128;
mod aes192;
mod aes256;

use aes128::AES128;
use aes192::AES192;
use aes256::AES256;

#[derive(Debug)]
pub enum AESError {
    InccorectSize
}

pub enum InitKey {
    AES128([u8; 16]),
    AES192([u8; 24]),
    AES256([u8; 32])
}

pub enum AES {
    AES128(AES128),
    AES192(AES192),
    AES256(AES256)
}

impl Default for AES {
    fn default() -> Self {
        AES::new()
    }
}

impl AES {
    pub fn new() -> Self {
        AES::AES256(AES256::new())
    }

    pub fn from_init_key(init_key: InitKey) -> Self {
        match init_key {
            InitKey::AES128(init_key) => AES::AES128(AES128::from_init_key(init_key)),
            InitKey::AES192(init_key) => AES::AES192(AES192::from_init_key(init_key)),
            InitKey::AES256(init_key) => AES::AES256(AES256::from_init_key(init_key)),
        }
    }

    pub fn from_hex(hex: &str) -> Result<Self, AESError> {
        Ok(AES::AES256(AES256::from_hex(hex)?))
    }

    pub fn init_key(&self) -> InitKey {
        match self {
            AES::AES128(aes) => InitKey::AES128(aes.init_key),
            AES::AES192(aes) => InitKey::AES192(aes.init_key),
            AES::AES256(aes) => InitKey::AES256(aes.init_key) 
        }
    }
}

impl CryptographicAlgorithm for AES {
    fn encrypt(&self, message: &[u8]) -> Vec<u8> {
        let message = message.chunks(16);

        let mut cipher = Vec::new();

        for slice in message {
            let mut vec = slice.to_vec();
            if slice.len() < 16 {
                while vec.len() < 16 {
                    vec.push(0);
                }
            }

            let mut matrix = vec_to_array16(vec).unwrap();

            match self {
                Self::AES128(aes) => aes.encryption_algorithm(&mut matrix),
                Self::AES192(aes) => aes.encryption_algorithm(&mut matrix),
                Self::AES256(aes) => aes.encryption_algorithm(&mut matrix),
            }

            cipher.append(&mut matrix.to_vec());
        }

        cipher
    }

    fn decrypt(&self, cipher: &[u8]) -> Vec<u8> {
        let cipher = cipher.chunks(16);

        let mut message = Vec::new();

        for slice in cipher {
            let mut vec = slice.to_vec();
            if slice.len() < 16 {
                while vec.len() < 16 {
                    vec.push(0);
                }
            }

            let mut matrix = vec_to_array16(vec).unwrap();

            match self {
                Self::AES128(aes) => aes.decryption_algorithm(&mut matrix),
                Self::AES192(aes) => aes.decryption_algorithm(&mut matrix),
                Self::AES256(aes) => aes.decryption_algorithm(&mut matrix),
            }

            message.append(&mut matrix.to_vec());
        }

        message
    }
}

// Key generation
fn rotate(byte: &mut [u8; 4]) {
    let temp = byte[0];

    for i in 0..3 {
        byte[i] = byte[i + 1];
    }

    byte[3] = temp;
}

fn rcon(mut byte: u8) -> u8 {
    let mut c: u8 = 1;

    if byte == 0 {
        return 0;
    }

    while byte != 1 {
        let b = c & 0x80;
        c <<= 1;
        if b == 0x80 {
            c ^= 0x1b;
        }
        byte -= 1;
    }

    c
}

fn schedule_core(bytes: &mut [u8; 4], i: u8) {
    rotate(bytes);
    for byte in bytes.iter_mut() {
        sbox(byte);
    }
    bytes[0] ^= rcon(i);
}


// Shared functions
fn add_key(matrix: &mut [u8; 16], key: &[u8]) {
    for i in 0..16 {
        matrix[i] ^= key[i];
    }
}

// Encryption
fn sbox(byte: &mut u8) {
    let sbox: [u8; 256] = [
        ,
    ];

    *byte = sbox[*byte as usize];
}

fn shift_rows(matrix: &mut [u8; 16]) {
    let mut temp = [0; 16];

    temp[0] = matrix[0];
    temp[1] = matrix[5];
    temp[2] = matrix[10];
    temp[3] = matrix[15];

    temp[4] = matrix[4];
    temp[5] = matrix[9];
    temp[6] = matrix[14];
    temp[7] = matrix[3];

    temp[8] = matrix[8];
    temp[9] = matrix[13];
    temp[10] = matrix[2];
    temp[11] = matrix[7];

    temp[12] = matrix[12];
    temp[13] = matrix[1];
    temp[14] = matrix[6];
    temp[15] = matrix[11];

    *matrix = temp;
}

fn mix_columns(matrix: &mut [u8]) {
    for i in 0..4 {
        let c = [
            matrix[i * 4],
            matrix[(i * 4) + 1],
            matrix[(i * 4) + 2],
            matrix[(i * 4) + 3],
        ];

        matrix[i * 4] = finite_field_multiplication(2, c[0])
            ^ finite_field_multiplication(3, c[1])
            ^ finite_field_multiplication(1, c[2])
            ^ finite_field_multiplication(1, c[3]);
        matrix[(i * 4) + 1] = finite_field_multiplication(1, c[0])
            ^ finite_field_multiplication(2, c[1])
            ^ finite_field_multiplication(3, c[2])
            ^ finite_field_multiplication(1, c[3]);
        matrix[(i * 4) + 2] = finite_field_multiplication(1, c[0])
            ^ finite_field_multiplication(1, c[1])
            ^ finite_field_multiplication(2, c[2])
            ^ finite_field_multiplication(3, c[3]);
        matrix[(i * 4) + 3] = finite_field_multiplication(3, c[0])
            ^ finite_field_multiplication(1, c[1])
            ^ finite_field_multiplication(1, c[2])
            ^ finite_field_multiplication(2, c[3]);
    }
}

// Decryption
fn reverse_sbox(byte: &mut u8) {
    let sbox: [u8; 256] = [
        ,
    ];

    *byte = sbox[*byte as usize];
}

fn reverse_shift_rows(matrix: &mut [u8; 16]) {
    let mut temp = [0; 16];

    temp[0] = matrix[0];
    temp[5] = matrix[1];
    temp[10] = matrix[2];
    temp[15] = matrix[3];

    temp[4] = matrix[4];
    temp[9] = matrix[5];
    temp[14] = matrix[6];
    temp[3] = matrix[7];

    temp[8] = matrix[8];
    temp[13] = matrix[9];
    temp[2] = matrix[10];
    temp[7] = matrix[11];

    temp[12] = matrix[12];
    temp[1] = matrix[13];
    temp[6] = matrix[14];
    temp[11] = matrix[15];

    *matrix = temp;
}

fn reverse_mix_columns(matrix: &mut [u8; 16]) {
    for i in 0..4 {
        let c = [
            matrix[i * 4],
            matrix[(i * 4) + 1],
            matrix[(i * 4) + 2],
            matrix[(i * 4) + 3],
        ];

        matrix[(i * 4)] = finite_field_multiplication(14, c[0])
            ^ finite_field_multiplication(11, c[1])
            ^ finite_field_multiplication(13, c[2])
            ^ finite_field_multiplication(9, c[3]);
        matrix[(i * 4) + 1] = finite_field_multiplication(9, c[0])
            ^ finite_field_multiplication(14, c[1])
            ^ finite_field_multiplication(11, c[2])
            ^ finite_field_multiplication(13, c[3]);
        matrix[(i * 4) + 2] = finite_field_multiplication(13, c[0])
            ^ finite_field_multiplication(9, c[1])
            ^ finite_field_multiplication(14, c[2])
            ^ finite_field_multiplication(11, c[3]);
        matrix[(i * 4) + 3] = finite_field_multiplication(11, c[0])
            ^ finite_field_multiplication(13, c[1])
            ^ finite_field_multiplication(9, c[2])
            ^ finite_field_multiplication(14, c[3]);
    }
}
